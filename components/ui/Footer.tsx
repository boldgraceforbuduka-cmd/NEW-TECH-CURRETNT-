        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-primary-foreground font-bold text-sm">TC</div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['News', 'AI', 'Programming', 'Cybersecurity', 'Startups'].map((c) => (
                <li key={c}>
                  <Link href={`/${c.toLowerCase()}`} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                    {c}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Newsletter', 'Tutorials', 'Reviews', 'Privacy Policy', 'Terms'].map((c) => (
                <li key={c}>
                  <Link href={`/${c.toLowerCase().replace(/ /g, '-')}`} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                    {c}
                  </Link>
                </li>
              ))}
