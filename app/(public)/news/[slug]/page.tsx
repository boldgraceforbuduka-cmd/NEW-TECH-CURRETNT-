      ...
      try {
        ...
        if (user && found) {
          const { error } = await supabase.from('reading_history').upsert(
            {
              user_id: user.id,
              article_id: found.id || found.url,
              article_category: found.category,
            },
            { onConflict: 'user_id, article_id' }
          );
          if (error) console.error('Failed to record reading history:', error);
        }
      } catch (error) {
        ...
        <div className="mb-6 sm:mb-8 p-3 sm:p-4 bg-card border border-border rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Listen to AI narration</p>
            </div>
          </div>
